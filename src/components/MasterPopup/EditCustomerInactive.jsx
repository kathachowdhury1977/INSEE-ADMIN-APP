...
export default function EditCustomerInactive(props) {
    const [open, setOpen] = React.useState(false);
    const [customerType, setCustomerType] = React.useState("");
    const [priority, setPriority] = React.useState("");
    const [visitCadence, setVisitCadence] = React.useState("");
    const dispatch = useDispatch();
    const myActionDetails = props.customerInactive;
    console.log("specialProjectI", myActionDetails);
    const updateExstingInactive = useSelector((state) => state.updatecustomerinactive);
    ...
    const handleSubmit = () => {
        ...
        let data = {
            ...
            "priority": priority ? priority : myActionDetails.priority
           

        }

        dispatch(eventActions.updateCustomerInActive({ 'data': data, 'customerId':  myActionDetails.id}));
        setCustomerType("");
        setPriority("");
        setVisitCadence("");
        props.setOpen(false);
    ...
    useEffect(() => {
        if (updateExstingInactive && !updateExstingInactive.loading &&
            (updateExstingInactive.updatecustomerinactive)) {
                dispatch(eventActions.getNonCustomerInActive());
            toast.success('Customer Inactive updated successfully', {
                position: 'top-right',
                autoClose: 4000,
    ...
    }, [updateExstingInactive]);


    useEffect(() => {
        return () => {
            dispatch(eventActions.updateCustomerInActive())
        }
    }, [])
                ...
                <DialogContent dividers>
                    <Typography gutterBottom>
                        <form>
                            ...
                            <div class="form-group">
                                <label for="customer_group">Priority</label>
                                <input type="text"
                                    class="form-control"
                                    placeholder="value"
                                    
                                    defaultValue={myActionDetails.priority} 
