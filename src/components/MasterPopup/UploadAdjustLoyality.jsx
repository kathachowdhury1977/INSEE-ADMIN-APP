...
export default function UploadAdjustLoyality(props) {
    const [open, setOpen] = React.useState(false);
    const dispatch = useDispatch();
    const [country, setCountry] = React.useState("TH");
    const [upload, setUpload] = React.useState("");
    const AdjustLoyalityAdmin = useSelector((state) => state.uploadadjustloyalityadmin);
    ...
    const handleSubmit = () => {
        dispatch(eventActions.uploadAdjustLoyalityAdmin(upload));
        setOpen(false);
    }

    useEffect(() => {
        if (!!AdjustLoyalityAdmin && !!AdjustLoyalityAdmin.uploadadjustloyalityadmin && AdjustLoyalityAdmin.uploadadjustloyalityadmin !== undefined) {
            dispatch(eventActions.getAdjuctLoyalityAdmin(1, 30));
            toast.success('Loyality Admin uploaded successfully', {
                position: "top-right",
                autoClose: 3000,
    ...
    useEffect(() => {
        return () => {
            dispatch(eventActions.uploadAdjustLoyalityAdmin())
        }
    }, [])
